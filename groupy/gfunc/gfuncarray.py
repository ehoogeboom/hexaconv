...
class GFuncArray(object):
    ...
    def __call__(self, sample_points):
        ...
        for i in range(si.shape[-1]):
            mask *= np.where(si[..., i] >= 0, 1, 0)
            mask *= np.where(si[..., i] < si.shape[i], 1, 0)

        si_masked = copy.copy(si)
        si_masked[mask == 0, :] = (0, ) * si.shape[-1]

        # If this fails, implementation is not correct.
        assert si.shape[:-1] == sample_points.shape

        inds = [Ellipsis] + [si_masked[..., i] for i in range(si.shape[-1])]
