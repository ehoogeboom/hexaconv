# Chainer implementation of the indexing kernels used in the G-gconv
...
_grad_index_group_func_str = \
    ...
    // atomicAdd for doubles is not implemented in cuda, so have to add it here
    __device__ char myAtomicAdd(char* address, char val)
    {{
        return val;
    }}
    ...
    {{
        CUPY_FOR(i, grad_output.size()) {{
            ...
            // atomicAdd for either double or floating points.
            {1}(&grad_input[index], grad_output[i]);
        }}
    }}
    """
...
_grad_index_group_func_kernel32 = compile_with_cache(
    _grad_index_group_func_str.format('float', 'atomicAdd')
).get_function('grad_indexing_kernel')
_grad_index_group_func_kernel64 = compile_with_cache(
    _grad_index_group_func_str.format('double', 'myAtomicAdd')
).get_function('grad_indexing_kernel')
