...
from groupy.gconv.make_gconv_indices import make_c4_z2_indices, make_c4_p4_indices,\
    make_d4_z2_indices, make_d4_p4m_indices, make_c6_z2_indices, make_d6_z2_indices, \
    make_c6_p6_indices, make_d6_p6m_indices
...
def test_transform_grad():
    for dtype, toll in [('float32', 1e-3), ('float64', 1e-10)]:
        ...
        check_transform_c6_z2_grad(dtype, toll)
        check_transform_c6_p6_grad(dtype, toll)
        check_transform_d6_z2_grad(dtype, toll)
        check_transform_d6_p6m_grad(dtype, toll)
...
def check_transform_c6_z2_grad(dtype='float64', toll=1e-10):
    inds = make_c6_z2_indices(ksize=3)
    w = cp.random.randn(3, 2, 1, 3, 3)
    check_transform_grad(inds, w, TransformGFilter, dtype, toll)

def check_transform_c6_p6_grad(dtype='float64', toll=1e-10):
    inds = make_c6_p6_indices(ksize=3)
    w = cp.random.randn(1, 2, 6, 3, 3)
    check_transform_grad(inds, w, TransformGFilter, dtype, toll)

def check_transform_d6_z2_grad(dtype='float64', toll=1e-10):
    inds = make_d6_z2_indices(ksize=3)
    w = cp.random.randn(3, 2, 1, 3, 3)
    check_transform_grad(inds, w, TransformGFilter, dtype, toll)

def check_transform_d6_p6m_grad(dtype='float64', toll=1e-10):
    inds = make_d6_p6m_indices(ksize=3)
    w = cp.random.randn(1, 2, 12, 3, 3)
    check_transform_grad(inds, w, TransformGFilter, dtype, toll)
